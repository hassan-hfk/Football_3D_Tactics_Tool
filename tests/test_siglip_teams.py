"""
test_siglip_teams.py — visual test: SigLIP embeddings + k-means for team
...
WHAT THIS DOES
  ...
  2. For every player/goalkeeper/referee detection, crops the bbox and embeds
     it with SigLIP (google/siglip-base-patch16-224 by default).
  3. Collects embeddings from the first detections (--bootstrap of them),
     ...
     and runs k-means with k=3 (two teams + referee/other) -- same idea as
     the current LAB approach, but on SigLIP features instead of raw colour.
...
GOAL
  Visually confirm: do same-kit players consistently get the same bbox colour,
  and does the referee end up in a separate cluster, EVEN when two kits are
  visually similar in raw colour (the cases that broke the LAB approach)?

USAGE
  python test_siglip_teams.py path/to/video.mp4 [--out out.mp4] [--frames 200]
...
NOTES
  - This is a STANDALONE diagnostic script, not wired into pipeline.py.
    Nothing here changes the running app; it's purely for visually checking
    whether SigLIP clustering looks more robust before we integrate it.
  - SigLIP runs on the configured device (mps/cuda/cpu, auto-detected).
  - For a quick first look, run on ~100-200 frames (--frames 100). The
    bootstrap clustering only needs a few dozen detections per kit colour.
"""
...
class SiglipEmbedder:
    def __init__(self, model_name, device):
        ...
        self.torch = torch
        self.device = device
        print(f"  Loading SigLIP: {model_name} (device={device})")
        self.processor = SiglipImageProcessor.from_pretrained(model_name)
        self.model = SiglipVisionModel.from_pretrained(model_name).to(device).eval()
    ...
    def embed(self, bgr_crops):
        ...
        feats = out.pooler_output.cpu().numpy()  # (N, D)
        return feats
...
def main():
    ...
    ap.add_argument("--out", default="siglip_teams_out.mp4", help="Output annotated video")
    ...
    ap.add_argument("--model", default=SIGLIP_MODEL_NAME, help="SigLIP model name")
    args = ap.parse_args()
    ...
    embedder = SiglipEmbedder(args.model, device)
    ...
    # PCA to a small number of dims keeps k-means stable on SigLIP's
